...
from core.keyboards.inline import profile, category_kb, education_kb
...
@router.message(Habit.habit_education, F.text.casefold().in_(
        ['изучать новый навык', 'читать', 'проходить учебный курс',
         'вести дневник', 'изучать новый язык', 'медитировать', 'назад']))
...
@router.message(Habit.habit_education)
async def incorrect_education_choice(message: Message, state: FSMContext):
    await message.answer('Выбери один из вариантов',
                         reply_markup=education_kb())


@router.message(Habit.habit_frequency)
async def education_frequency(message: Message, state: FSMContext):
...
@router.message(Habit.habit_report, F.text.casefold().in_(['фотоочет',
                                                          'текст']))
async def education_report(message: Message, state: FSMContext):
...
async def incorrect_education_report(message: Message, state: FSMContext):
    await message.answer('Выбери один из вариантов',
                         reply_markup=profile(['Фотоочет', 'Текст']))
