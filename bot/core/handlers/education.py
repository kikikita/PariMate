...
from core.keyboards.inline import profile, category_kb, education_kb, \
    report_kb, mate_kb
from core.utils.states import Habit, Education
...
async def healh_choice(message: Message, state: FSMContext):
    if message.text in 'Назад':
        ...
        await state.set_state(Education.habit_frequency)
...
@router.message(Education.habit_frequency)
async def sport_frequency(message: Message, state: FSMContext):
    if message.text in 'Назад':
        ...
        await state.set_state(Education.habit_report)
...
@router.message(Education.habit_report,
...
async def sport_report(message: Message, state: FSMContext):
    if message.text in 'Назад':
        ...
        await state.set_state(Education.habit_mate_sex)
...
@router.message(Education.habit_mate_sex)
