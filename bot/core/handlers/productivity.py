...
from core.keyboards.inline import profile, category_kb, productivity_kb, \
    report_kb, mate_kb
from core.utils.states import Habit, Productivity
...
async def healh_choice(message: Message, state: FSMContext):
    if message.text in 'Назад':
        ...
        await state.set_state(Productivity.habit_frequency)
...
@router.message(Productivity.habit_frequency)
async def sport_frequency(message: Message, state: FSMContext):
    if message.text in 'Назад':
        ...
        await state.set_state(Productivity.habit_report)
...
@router.message(Productivity.habit_report,
...
async def sport_report(message: Message, state: FSMContext):
    if message.text in 'Назад':
        ...
        await state.set_state(Productivity.habit_mate_sex)
...
@router.message(Productivity.habit_mate_sex)
