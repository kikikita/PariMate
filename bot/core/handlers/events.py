...
async def on_user_join(event: ChatMemberUpdated):
    ...
    time_string = (str(user["habit_notification_time"])[1:-1]
                   .replace("'", ""))
    await event.answer(
        f'Привет, <b>{user["name"]}</b>! В этом чате, вы с напарником будете' +
    ...
    await event.answer(
        f'Твоя цель - <b>{user["habit_choice"].lower()} ' +
        f'{user["habit_frequency"]} раз(-а) в неделю</b>' +
        f'\nПо этим дням: {days_string}\nВ это время: {time_string}'
        '\n\nВ течение ближайшей недели присылай сюда фото или видео-' +
        'подтверждения того, как внедряешь привычку в свою жизнь. ' +
        'Для этого жми /pari -> "Подтвердить выполнение привычки"' +
        f'{text}' +
